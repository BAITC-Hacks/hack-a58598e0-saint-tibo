...
**В этом срезе STT ещё не подключён:** корректная запись доходит до transcribe
и заканчивается `failed / transcription_unavailable`. Это не готовый AI-результат.
Подключение настоящего STT и версий транскрипта — следующий срез #11.
`result_version_id` пока null, succeeded не публикуется.
