...
Snapshot: 2026-09-23. [PR #127](https://github.com/BAITC-Hacks/hack-a58598e0-saint-tibo/pull/127) shipped the requested workspace PR #124 and its draft/export guards to both environments as `3676470`. The next integrated player workflow is Ivan's `12abf23`, merged through `fd8e936`: the editor now opens inside /player. The complete GPU/API/browser proof baseline is main `1adbcb5` from [PR #119](https://github.com/BAITC-Hacks/hack-a58598e0-saint-tibo/pull/119), with equivalent dev tree `420aac5`. These frontend follow-ups preserve that processing runtime. Read the [latest #94 receipt](https://github.com/BAITC-Hacks/hack-a58598e0-saint-tibo/issues/94) for the exact deployed SHA and final browser result; a push alone never deploys. Production: [saint-tibo.win](https://saint-tibo.win).

## Delivered

The canonical flow is recording → private GPU transcription → anonymous speaker intervals → private GPU draft actions/summary → explicit review → PDF/DOCX → curator reminders. Ready published teammate work through the release is integrated, including persistent playback, speaker filtering/solo playback, person briefings and the saved meeting overview. [DEMO.md](../DEMO.md) contains the dedicated ordinary demo accounts and walkthrough. /player shows the saved protocol with its draft/confirmed status and embeds the expandable editor for result-version selection, evidence editing and historical export; /workspace redirects there. The embedded editor's processing button explicitly requests transcription; /meetings offers the full GPU pipeline. Reconnect refresh cannot discard its unsaved review, and export is disabled while dirty/saving/reloading. The obsolete public-MP3 player branch remains deliberately unmerged.
