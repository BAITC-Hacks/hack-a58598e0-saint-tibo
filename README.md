...
**Backend поддерживает путь от записи до транскрипта:** настоящая авторизация,
совещания и участники, загрузка файла или частей, приватное воспроизведение
с перемоткой, очередь заданий в PostgreSQL и локальный STT с сохраняемыми
версиями и таймкодами. API-клиент генерируется из OpenAPI.
`target_stage=transcribe` означает готовый транскрипт; диаризация,
поручения и саммари остаются следующими этапами. Модуль рендера PDF/DOCX
подготовлен, его endpoint и подключение к проверенному протоколу ещё впереди.

UI, дизайн и сайт отдельно разрабатывает Артём. Текущее рабочее пространство
RU/KK/EN и компоненты плеера/захвата не заменяют готовый пользовательский
путь. Состояние API и UI следует оценивать раздельно.
Контракт и ограничения загрузки — в [документации](docs/recording-storage.md),
очередь — в [processing-jobs.md](docs/processing-jobs.md),
модель и чтение транскрипта — в [transcription.md](docs/transcription.md).

Аудио и текст совещаний должны обрабатываться в закрытом контуре.
Внешние облачные API для этих данных запрещены исходными условиями кейса.
Временный API-режим извлечения черновика из реальной транскрипции разрешён владельцем
проекта; при его включении текст разговора передаётся провайдеру. Настройка и путь
возврата к локальной модели описаны в [документе извлечения](docs/api-extraction.md).
...
[инструкции STT](docs/transcription.md). На dev Данила модель уже установлена;
остальным окружениям она нужна до первого задания. Наличие HTTPS и входа
само по себе не означает, что модель установлена.
